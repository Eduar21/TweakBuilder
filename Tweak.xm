...
// ── IOKit Monitor ─────────────────────────────────────────────
// Hookea IOConnectCallMethod para ver selectores de llamadas al kernel
typedef kern_return_t (*ioconnect_fn_t)(
    mach_port_t, uint32_t,
    const uint64_t *, uint32_t,
    const void *, size_t,
    uint64_t *, uint32_t *,
    void *, size_t *);
static ioconnect_fn_t  g_orig_ioconnect  = NULL;
static void          **g_ioconnect_slot   = NULL;
static BOOL            g_iokit_log        = NO;
...
// ════════════════════════════════════════════
//  IOKit MONITOR — hookea IOConnectCallMethod
//  Loguea selector + args de cada llamada al kernel via IOKit
// ════════════════════════════════════════════

// Busca el slot GOT de fn en cualquier imagen no-cache cargada
static void **iokit_find_got_any(void *target) {
    uint32_t imgN = _dyld_image_count();
    for (uint32_t ii = 0; ii < imgN; ii++) {
        const char *imgname = _dyld_get_image_name(ii);
        if (!imgname) continue;
        if (strstr(imgname, "/usr/lib/system")) continue;

        const struct mach_header_64 *mh =
            (const struct mach_header_64*)_dyld_get_image_header(ii);
        if (!mh || mh->magic != MH_MAGIC_64) continue;
        intptr_t slide = _dyld_get_image_vmaddr_slide(ii);
            ...
            if (cmd->cmd == LC_SEGMENT_64) {
                ...
                for (uint32_t k = 0; k < seg->nsects; k++, sect++) {
                    ...
                    if (type != S_NON_LAZY_SYMBOL_POINTERS &&
                        type != S_LAZY_SYMBOL_POINTERS) continue;
                    void **ptrs = (void **)(sect->addr + slide);
                    NSUInteger cnt = sect->size / sizeof(void*);
                    for (NSUInteger m = 0; m < cnt; m++) {
                        if (ptrs[m] == target) return &ptrs[m];
                    }
                }
            }
            lc += cmd->cmdsize;
...
// Trampoline — loguea la llamada y pasa al original
static kern_return_t iokit_trampoline(
    mach_port_t conn, uint32_t sel,
    ...
    if (g_iokit_log) {
        NSMutableString *msg = [NSMutableString stringWithFormat:
            @"[IOKit] conn=0x%x sel=0x%02x", conn, sel];
        for (uint32_t i = 0; i < inCnt && i < 4; i++)
            [msg appendFormat:@" a%u=0x%llx", i, in[i]];
        if (inSCnt > 0)
            [msg appendFormat:@" struct=%zub", inSCnt];
        LOGL(msg);
    }
    return g_orig_ioconnect(conn, sel,
        in, inCnt, inS, inSCnt,
        out, outCnt, outS, outSCnt);
}

// Instalar/desinstalar el hook de IOConnectCallMethod
static BOOL iokit_hook_install(void) {
    if (g_orig_ioconnect) return YES;  // ya instalado
    void *fn = dlsym(RTLD_DEFAULT, "IOConnectCallMethod");
    if (!fn) {
        // Intentar cargar IOKit explícitamente
        dlopen("/System/Library/Frameworks/IOKit.framework/IOKit",
               RTLD_NOW | RTLD_GLOBAL);
        fn = dlsym(RTLD_DEFAULT, "IOConnectCallMethod");
    }
    if (!fn) return NO;
    g_ioconnect_slot = iokit_find_got_any(fn);
    if (!g_ioconnect_slot) return NO;
    g_orig_ioconnect = (ioconnect_fn_t)fn;
    got_write(g_ioconnect_slot, (void*)iokit_trampoline);
    return YES;
}

static void iokit_hook_remove(void) {
    if (!g_ioconnect_slot || !g_orig_ioconnect) return;
    got_write(g_ioconnect_slot, (void*)g_orig_ioconnect);
    g_orig_ioconnect = NULL;
    g_ioconnect_slot = NULL;
}
...
// IOKit monitor: hookea IOConnectCallMethod y loguea sel + args
+ (void)iokitToggle {
    g_iokit_log = !g_iokit_log;
    UIButton *b = (UIButton*)[g_panel viewWithTag:215];
    if (g_iokit_log) {
        BOOL ok = iokit_hook_install();
        if (!ok) {
            g_iokit_log = NO;
            LOGL(@"[IOKit] ✗ no encontré IOConnectCallMethod en GOT "
                 @"— probá en otra app");
            return;
        }
        [b setTitle:@"● IOKit" forState:UIControlStateNormal];
        b.backgroundColor = [UIColor colorWithRed:0.1 green:0.4 blue:0.8 alpha:1];
        LOGL(@"[IOKit] ON — loguea TODAS las llamadas al kernel via IOKit");
        LOGL(@"  formato: conn=X sel=0xNN a0=... a1=...");
        LOGL(@"  buscá el sel que aparece al hacer la operación Metal");
    } else {
        iokit_hook_remove();
        [b setTitle:@"IOKit" forState:UIControlStateNormal];
        b.backgroundColor = [UIColor colorWithRed:0.1 green:0.2 blue:0.5 alpha:1];
        LOGL(@"[IOKit] OFF");
    }
}
...
+ (void)hookRun {
    NSString *c = [g_hkCls.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
    NSString *s = [g_hkSel.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
    NSString *v = [g_hkVal.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
    [g_panel endEditing:YES];
...
+ (void)dDump {
    NSString *n = [g_dumpField.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
...
+ (void)dGot {
    NSString *f = [g_dumpField.text stringByTrimmingCharactersInSet:NSCharacterSet.whitespaceAndNewlineCharacterSet];
    [g_dumpField resignFirstResponder];
    dispatch_async(dispatch_get_global_queue(
        DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
static void build_ui(void) {
  ...
  forControlEvents:UIControlEventValueChanged];
    ...
    // ── Controles L: TRACE / REC / IOKit / CLEAR / SAVE / COPY ──
        ...
        CGRectMake(0, ctl_y, pw, 40)];
    {
        ...
        UIButton *ib = mkbtn(@"IOKit", @selector(iokitToggle),
            CGRectMake(10+2*(w+4), 0, w, 32),
            [UIColor colorWithRed:0.1 green:0.2 blue:0.5 alpha:1]);
        ib.tag = 215;
        ib.titleLabel.font = [UIFont monospacedSystemFontOfSize:9
            weight:UIFontWeightBold];
        [g_ctlL addSubview:ib];
        [g_ctlL addSubview:mkbtn(@"CLEAR", @selector(clearL),
            CGRectMake(10+3*(w+4), 0, w, 32), cGray)];
        [g_ctlL addSubview:mkbtn(@"SAVE", @selector(saveL),
            CGRectMake(10+4*(w+4), 0, w, 32), cBlue)];
        [g_ctlL addSubview:mkbtn(@"COPY", @selector(copyL),
            CGRectMake(10+5*(w+4), 0, w, 32), cSlate)];
    }
